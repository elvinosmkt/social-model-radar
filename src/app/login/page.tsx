            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                ...
                <div className="glass-effect rounded-3xl p-10 border border-white/8 shadow-2xl">
                    ...
                    <div className="flex flex-col items-center mb-8">
                        ...
                        <h1 className="text-2xl font-outfit font-bold premium-gradient-text tracking-tight">DWS SCOUTER</h1>
                        <p className="text-xs text-text-secondary uppercase tracking-[0.2em] mt-1">Digital Web Scouter</p>
                    </div>
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="seu@email.com" required
                                className="w-full bg-white/5 border border-white/10 focus:border-primary/50 rounded-xl px-4 py-3 text-sm outline-none transition-all placeholder:text-text-secondary/40" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[11px] font-bold uppercase tracking-widest text-text-secondary">Senha</label>
                            <div className="relative">
                                <input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required
                                    className="w-full bg-white/5 border border-white/10 focus:border-primary/50 rounded-xl px-4 py-3 pr-12 text-sm outline-none transition-all placeholder:text-text-secondary/40" />
